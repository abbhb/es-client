<template>
  <div class="field-condition-item">
    ...
    <a-select v-model="condition.field" allow-search allow-create allow-clear placeholder="请选择查询字段"
      ...
      <a-option v-for="(field, idx) in fields" :key="idx" :label="field.label" :value="field.value" :title="field.label"/>
    </a-select>
    <!-- 选择查询条件 -->
    <!-- TODO: 此处还需处理，对于不同类型，需要不同查询条件 -->
    <a-select v-model="condition.condition" filterable placeholder="请选择查询条件"
              style="margin-left: 10px;width: 140px;" allow-search>
      <a-option v-for="option in BaseQueryConditionOption" :key="option.value" :label="option.label"
                :value="option.value"/>
    </a-select>
    <!-- 值 -->
...
import {BaseQuery, BaseQueryConditionOption, getDefaultBaseQuery} from "@/entity/BaseQuery";
